...
class SMCTHUG:
    def __init__(self, N, d, ystar, logprior, ϵmin=None, pmin=0.2, pter=0.01, tolscheme='unique', η=0.9, mcmc_iter=5, iterscheme='fixed', propPmoved=0.99, δ0=0.2, minstep=0.1, maxstep=100.0, a_star=0.3, B=5, manual_initialization=False, maxiter=300, maxMCMC=10, precondition=False, thug=True, force_hug=False):
        ...
        assert not precondition, "Preconditining not implemented in this version."
        self.thug = thug
        ...
        if thug:
            if not force_hug:   ##### THUG
                if precondition:
                    raise NotImplementedError("In this version of SMC, no preconditioning is implemented.")
                else:
                    print("### MCMC kernel: THUG.")
                    self.MCMCkernel = lambda *args: wrapMCMCoutput(*(HugTangential(*args)))
                    self.MCMC_args  = lambda x0, N: (x0, self.B * self.step_sizes[-1], self.B, N, self.α, self.q, self.logpi, self.jac_f)
                ...
                if precondition:
                    ...
                    self.MCMCkernel = lambda *args: wrapMCMCoutput(*(HugTangential(*args)))
                    self.MCMC_args  = lambda x0, N: (x0, self.B * self.step_sizes[-1], self.B, N, 0.0, self.q, self.logpi, self.jac_f)
                    self.estimateΣ  = lambda: eye(self.d)
    ...
    def logpi(self, θ):
        """Target distribution. For BIP."""
        return self.logprior(θ) + MVN.logpdf(self.f(θ), mean=self.ystar, cov=self.EPSILON[-1]**2)
    ...
    @staticmethod
    def jac_f(ξ):
        """Computes the jacobian of f(xi). Used by HUG/THUG."""
        raise NotImplementedError
