...
def AlternatingManifoldHMC_AdaptiveNoKernel(x0, N, n, m, Sigma, mu, T, epsilon, M, s=0.5, tol=1.48e-08, a_guess=1, ap_star=0.6):
    """
    ...
    while len(samples) < N:
        ...
        zappa_samples = zappa_adaptive(
            x_hmc, 
            RotatedEllipse(mu, Sigma, z), 
            logf, m, s, tol, a_guess, 
            ap_star=ap_star, update_scale=update_scale_sa
        )
...
samples = AlternatingManifoldHMC_AdaptiveNoKernel(x0=x0, N=N, n=n, m=m, Sigma=Sigma, mu=mu, T=T, epsilon=epsilon, M=M, s=s, ap_star=ap_star)
end_time = time.time()
np.save("zhmcaa1_samples", samples)
np.save("zhmcaa1_time", end_time - start_time)
