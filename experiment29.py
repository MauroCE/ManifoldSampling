# Experiment 29: Is Hug+Hop better than Hug alone? I test this on a example with uniform kernel, uniform prior
# and manifold being the usual contour of a MVN.
import numpy as np
...
def log_uniform_kernel(xi, epsilon, target, z0):
...
def log_abc_posterior_all(xi, epsilon):
    """Log density of ABC posterior. Product of (param-latent) prior and uniform kernel."""
...
def experiment(x00, T, B, N, lam, kappa, epsilon, q, target, z0, nlags):
    ...
    etho = 0.0
    ### Redefine functions
    log_kernel = lambda xi, epsilon: log_uniform_kernel(xi, epsilon, target, z0)
    log_post   = lambda xi: logprior_uniform(xi) + log_kernel(xi, epsilon)
...
if __name__ == "__main__":
    ...
    Ts = [10, 1, 0.1, 0.01] #[10, 1, 0.1, 0.01] #[7, 5, 3, 1, 0.1, 0.01]
    epsilons = [0.1, 0.001, 0.00001] #[0.1, 0.001, 0.00001, 0.0000001]
    ...
    for i in range(n_runs):
        ...
        for j, epsilon in enumerate(epsilons):
            ...
            for k, T in enumerate(Ts):
                ...
                THETA_ESS_HH[i, j, k]   = out['HH']['ESS_T']
                U_ESS_HH[i, j, k]       = out['HH']['ESS_U']
                ESS_JOINT_HH[i, j, k]   = out['HH']['ESS_J']
                ...
                THETA_AC_HH[i, j, k, :] = out['HH']['AC_T']
                U_AC_HH[i, j, k, :]     = out['HH']['AC_U']
                # Store HUG results
                THETA_ESS_HO[i, j, k]   = out['HO']['ESS_T']
                U_ESS_HO[i, j, k]       = out['HO']['ESS_U']
                ESS_JOINT_HO[i, j, k]   = out['HO']['ESS_J']
                ...
                THETA_AC_HO[i, j, k, :] = out['HO']['AC_T']
                U_AC_HO[i, j, k, :]     = out['HO']['AC_U']

    print("Total time: ", time.time() - initial_time)

    # Save results (29: uniform kernel, uniform prior. 29b: uniform kernel, gaussian prior)
    folder = "experiment29/" #"experiment29/"
