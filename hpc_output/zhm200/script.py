import numpy as np
from numpy import log, zeros, eye, outer
...
def HopKernel(x, grad_log_pi, l, k, logpi):
    ...
    # B^{1/2}
    B_sqrt = (mu*eye(d) + (l - mu) * outer(gxhat, gxhat)) / np.sqrt(1 + gx_norm**2)
...
def MixtureZappaHop(x0, alpha, N, m, Sigma, mu, grad_log_pi, l, k, s=0.5, tol=1.48e-08, a_guess=1):
    """
    Mixture of Hop and Zappa. Non adaptive. It always uses 1 Hop step.
    """
    ...
    while len(samples) < N: 
        ...
        # With probability alpha do 1 (ONE!! NOT n) steps of HMC 
        if np.random.rand() <= alpha:
            new_samples = HopKernel(x, grad_log_pi, l, k, target.logpdf).reshape(1, -1)  # Will always be 1 sample
...
samples = MixtureZappaHop(x0, alpha=alpha, N=N, m=m, Sigma=Sigma, mu=mu, grad_log_pi=grad_log_pi, l=l, k=k, s=s)
end_time = time.time()
np.save("zhm200_samples", samples)
np.save("zhm200_time", end_time - start_time)
