...
def AlternatingZappaHop(x0, N, n, m, Sigma, mu, grad_log_pi, l, k, s=0.5, tol=1.48e-08, a_guess=1):
    """
    Alternates between Hop and Zappa.
    """
    ...
    while len(samples) < N:
        ...
        # m steps of Zappa Kernel
        zappa_samples = zappa_sampling(x, RotatedEllipse(mu, Sigma, target.pdf(x)), logf, logp, m, s, tol, a_guess)
        samples = np.vstack((samples, zappa_samples))
        x = zappa_samples[-1]
...
samples = AlternatingZappaHop(x0, N=N, n=n, m=m, Sigma=Sigma, mu=mu, grad_log_pi=grad_log_pi, l=l, k=k, s=s)
end_time = time.time()
np.save("zha1_samples", samples)
np.save("zha1_time", end_time - start_time)
