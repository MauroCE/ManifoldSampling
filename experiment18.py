# Experiment 18: HUG + HOP vs HUG-AR + HOP in LFI setting with uniform kernel and uniform prior.
# We use the degenerate AR process.
import numpy as np
from numpy import zeros, diag, eye, log, sqrt, vstack, mean, save
from numpy.linalg import solve
from scipy.stats import multivariate_normal as MVN
from tangential_hug_functions import Hop, HugStepEJSD, HugARStepEJSD
...
if __name__ == "__main__":
    ...
    RMSE_HUG_AR = zeros((n_runs, n_epsilons, n_probs))    
    EJSD_HUG_AR = zeros((n_runs, n_epsilons, n_probs))             
    G_EJSD_HUG_AR = zeros((n_runs, n_epsilons, n_probs))           
    T_EJSD_HUG_AR = zeros((n_runs, n_epsilons, n_probs))
    A_HOP_HUG_AR = zeros((n_runs, n_epsilons, n_probs))    
    N_UNIQUE_HUG_AR = zeros((n_runs, n_epsilons, n_probs))        
    ...
    for j, epsilon in enumerate(epsilons):
        ...
        for i in range(n_runs):
            ...
            for k, prob in enumerate(probs):
                hug_ar, ahug_ar, ahop, e, eg, et = runHH_AR(x0, lam, N, q, prob)
