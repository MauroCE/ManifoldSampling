...
# Investigates robustness to T. We keep epsilon fixed and change T. Does not use large Hessian.
import numpy as np
...
Ts = [0.1, 1.0, 5.0, 10.0, 20.0]

# HUG
uESS_HUG = zeros((n_runs, len(Ts)))        # Univariate ESS for \theta chain
ESS_LOGPI_HUG = zeros((n_runs, len(Ts)))   # ESS for joint ABC posterior log p(\xi | y)
A_HUG = zeros((n_runs, len(Ts)))           # Acceptance probability
RMSE_HUG = zeros((n_runs, len(Ts)))        # Root Mean Squared Error

# THUG
uESS_THUG = zeros((n_runs, len(Ts), len(alphas)))  
ESS_LOGPI_THUG = zeros((n_runs, len(Ts), len(alphas))) 
A_THUG = zeros((n_runs, len(Ts), len(alphas)))            
RMSE_THUG = zeros((n_runs, len(Ts), len(alphas)))    
...
save("experiment11c/TS.npy", Ts)
save("experiment11c/ALPHAS.npy", alphas)

save("experiment11c/ESS_HUG.npy", uESS_HUG)
save("experiment11c/ESS_LOGPI_HUG.npy", ESS_LOGPI_HUG)
save("experiment11c/A_HUG.npy", A_HUG)
save("experiment11c/RMSE_HUG.npy", RMSE_HUG)

save("experiment11c/ESS_THUG.npy", uESS_THUG)
save("experiment11c/ESS_LOGPI_THUG.npy", ESS_LOGPI_THUG)
save("experiment11c/A_THUG.npy", A_THUG)
save("experiment11c/RMSE_THUG.npy", RMSE_THUG)