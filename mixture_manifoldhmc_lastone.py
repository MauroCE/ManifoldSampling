...
def MixtureManifoldHMC_LastOne(x0, alpha, N, n, m, Sigma, mu, T, epsilon, M, s=0.5, tol=1.48e-08, a_guess=1):
    """
    ...
    while len(samples) < N: 
        ...
        if np.random.rand() <= alpha:
            new_sample = GaussianTargetHMC(q0=x, n=n, M=M, T=T, epsilon=epsilon, Sigma=Sigma, mu=mu).sample()[-1] ###[1:] LAST ONE
            
        # With probability 1 - alpha do m steps of Zappa's algorithm
        else:
            # Keep only the last one! Do not keep the whole shebang
            new_sample = zappa_sampling(x, RotatedEllipse(mu, Sigma, z), logf, logp, m, s, tol, a_guess)[-1] ### LAST ONE!
