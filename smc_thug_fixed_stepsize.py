"""
Same as smc_thug.py. However here we follow Christophe's advice of keeping the
step size constant and only adapting α.
"""
import numpy as np
...
class SMCTHUG:
    def __init__(self, N, d, ystar, logprior, ϵmin=None, pmin=0.2, pter=0.01, tolscheme='unique', η=0.9, mcmc_iter=5, iterscheme='fixed', propPmoved=0.99, δ0=0.2, a_star=0.3, B=5, manual_initialization=False, maxiter=300, maxMCMC=10, precondition=False, thug=True, force_hug=False, αmax=0.999, αmin=0.01, both_stopping_criterions=False, verbose=True):
        ...
        if not self.both_stopping_criterions:
            ...
            self.stopping_criterion = min_tolerance_and_acc_prob
    ...
    def sample(self):
        ...
        while self.stopping_criterion():
            ...
            verboseprint("ϵ = {:.10f}\t N unique starting: {}".format(round(self.EPSILON[-1], 5), self.n_unique_starting[-1]))

            # COMPUTE COVARIANCE MATRIX (d x d) from previous weights
            self.Σ = self.estimateΣ()

            # METROPOLIS-HASTINGS - MOVE ALIVE PARTICLES
            verboseprint("Metropolis-Hastings steps: ", self.MCMC_iter[-1])
            ...
            try:
                ...
                verboseprint("Average Acceptance Probability: {:.4f}".format(self.accprob[-1]))
            ...
            verboseprint("Stepsize used in next SMC iteration: {:.4f}".format(self.step_sizes[-1]))

            # TUNE SQUEEZING PARAMETER FOR THUG
            self.update_α(self.accprob[-1], self.t)
            self.ALPHAS.append(self.α)
            verboseprint("Alpha used in next SMC iteration: {:.4f}".format(self.α))
