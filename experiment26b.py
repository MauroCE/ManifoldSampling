...
def logprior(thetau):
    ...
    with np.errstate(divide='ignore'):
        return log(((0 <= thetau[:3]) & (thetau[:3] <= 10)).all().astype('float64')) + ndist.logpdf(thetau[3:]).sum()
...
n_T = len(Ts)
n_runs = 2                          # Number of runs for each setting combination
rng_seed = 1234
